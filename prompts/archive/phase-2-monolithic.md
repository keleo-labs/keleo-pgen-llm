# Phase 2: JSON Translation and Schema Mapping

**Execution Context:** This prompt is designed for use within the `/translate-methodology` skill in the `keleo-pgen-llm` repository. All required resources are available as local files. This phase receives the output from Phase 1 (research report) and transforms it into schema-compliant JSON.
...
Your objective is to parse a structured research report (created by Phase 1) and translate it into executable JSON that conforms exactly to the schema at `deps/language.schema.json`.

**CRITICAL DIRECTIVES:**

* **MANDATORY SINGLE-SHOT EXECUTION:** Process the complete report and generate final JSON in one response
* **ANTI-HALLUCINATION:** The baseline at `deps/platform-adoption-kernel.json` defines your reality. If an element isn't in the baseline, it doesn't exist
...
Before beginning translation, read the following files from the repository:

1. **The Baseline Framework** - `deps/platform-adoption-kernel.json`
   
   Load and parse to extract exact names (case-sensitive):
   - Focus names: "Value", "Solution", "Endeavor"
   - Alpha names and their State names
   - ActivitySpace names
...
2. **The JSON Schema** - `deps/language.schema.json`
...
3. **The Phase 1 Research Report** - The human-readable structured report to translate
   
   Generated by Phase 1 analyst, contains all methodology content in parseable prose format

4. **Practice Dependencies (Optional)** - Other practice JSON files if the report references dependencies
...
```
{
  ...
  "citations": Citation[] (optional)  // Aggregated citations from method and practices
}
```
...
```
{
  ...
  "citations": Citation[] (optional),  // Bibliographic references
  // Standard metadata
  "authors": string[],
  "createdAt": string,
  "updatedAt": string,
...
```
{
  ...
  "citationNames": string[] (optional)  // Citation.name references for sources supporting this narrative
}
```
...
```
{
  "name": string,  // Title of the work (serves as citation identifier)
  "description": string,  // Brief description of the source
  "authors": string[],  // REQUIRED minimum 1 - person, group, or organization
  "date": string,  // REQUIRED - publication date (year or "n.d.")
  "source": string  // REQUIRED - publisher, journal, or URL
}
```
...
8. **Citations and References** → Extract Citation objects from References section
9. **Appendix: Terminology Mapping** → Extract PracticeElementAliases
...
**CRITICAL:** Every practice MUST extract citations from the "Citations and References" section of the report.
...
Extract to Citation objects:
```json
{
  "citations": [
    {
      "name": "AWS Well-Architected Framework",
      "description": "Authoritative framework from AWS providing architectural best practices for cloud workloads",
    ...
    },
    {
      ...
      "description": "Foundational text on reliable software releases through automated build, test, and deployment",
...
1. **Parse the References section** to identify all sources in APA7 format
2. **Extract bibliographic elements:**
   - **name**: The title of the work (remove italics, this becomes the citation identifier)
   - **description**: Brief description of the source and its relevance (1-2 sentences)
   - **authors**: Array of author names or organization name (keep formatting like "Lastname, F.")
   - **date**: Publication year or "n.d." if not available
   - **source**: URL, publisher, or publication venue

3. **Place citations at practice level:**
   - All citations go in the Practice.citations array or Method.citations array
   - Citations can be referenced by name in narratives using the citationNames array

4. **Link citations to narratives:**
   - When a narrative references specific sources, include those citation names in the narrative's citationNames array
   - Example: A narrative describing techniques from the AWS Well-Architected Framework would include `"citationNames": ["AWS Well-Architected Framework"]`

5. **Validation:**
   - Every practice should have multiple citations (minimum 3, recommend 5-15)
...
**Example narrative with citation references:**
  ...
  "narratives": [
    {
      ...
      "description": "Technique for balancing security governance with developer autonomy",
      "narrativeName": "Establishing Effective Guardrails",
      "narrativeTypeName": "The STAR Format",
      "narrativeContexts": [
      ...
      "citationNames": ["AWS Well-Architected Framework", "Continuous Delivery"]
    }
  ]
}
```
...
Read `deps/platform-adoption-kernel.json` and extract all exact element names:

1. **Load baseline Alpha names from the alphas array:**
   
   Examples: "Opportunity", "Platform Value And Economics", "Stakeholders", "Platform", "Requirements", "System", "Platform Governance", "Platform Asset", "Platform Consumption Interface", "Team", "Way Of Working", "Work", "Platform Risk And Compliance", "Organizational Change"

2. **Load baseline ActivitySpace names from the activitySpaces array:**
   
   Examples: "Assess Business Value", "Manage Platform Economics", "Architect and Build the Foundation", "Develop the Golden Paths", "Establish Secure Guardrails", "Integrate Toolchain Services", "Operate and Evolve the System", "Structure Organizational Topologies", "Drive Cultural Transformation", "Manage Work Execution", etc.

3. **Load baseline Competency names from the competencies array:**
   
   Exact names: "Analysis", "Engineering", "Leadership", "Management", "Test", "Usage"

4. **Load CompetencyLevel names from each competency's levels array:**
   
   Exact names: "Basic", "Applies", "Masters", "Adapts", "Innovating"

5. **Load NarrativeType names from the narrativeTypes array:**
   
   Examples: "The STAR Format", "The Hero's Journey", "The Three-Act Structure & StoryBrand", "Micro-Narratives (ABT)", "User story", "Epic", "Lifecycle", "Essay Narrative", "Report Narrative"

6. **Load NarrativeElement names from each NarrativeType's narrativeElements array:**
   
   Examples vary by narrative type (e.g., STAR has "Situation", "Task", "Action", "Result")

7. **For every symbolic reference in the JSON you generate:**
   - alphaName in AlphaContribution → MUST match a baseline or declared Alpha exactly
   - stateName in AlphaContribution → MUST match a State name exactly within that Alpha
   - activitySpaceName → MUST match baseline ActivitySpace exactly
   - competencyName → MUST match baseline Competency exactly
   - competencyLevelName → MUST match CompetencyLevel name exactly
   - narrativeTypeName → MUST match baseline NarrativeType exactly
   - narrativeElementName → MUST match NarrativeElement name within that NarrativeType exactly
...
```json
{
  ...
  "createdAt": "YYYY-MM-DD",  // Use current date (2026-05-19)
  "updatedAt": "YYYY-MM-DD",  // Use current date
  "version": "1.0",
...
- [ ] Practice includes multiple Citation objects (minimum 3, recommend 5-15)
- [ ] Each Citation has all required fields (name, description, authors array, date, source)
...
- [ ] Citations placed in Practice.citations array or Method.citations array
- [ ] Narratives reference citations using citationNames array where appropriate
...
1. Use the Read tool to load `deps/platform-adoption-kernel.json`:
   - Extract all Alpha names and their State names
   - Extract all ActivitySpace names
   - Extract all Competency names and CompetencyLevel names
   - Extract all NarrativeType names and their NarrativeElement structures
   - Store these in memory for exact name matching

2. Use the Read tool to review `deps/language.schema.json`:
   - Understand required vs optional fields
   - Note minimum array lengths
   - Verify property names and data types

3. Read the Phase 1 research report provided (markdown file)

**As you translate:**
- Parse systematically: structure → content → validation
- Use ONLY exact names from the baseline (zero tolerance for fuzzy matching)
- Extract ALL content (no truncation, no placeholders)
- Apply intelligent parsing for narratives (identify type from structure)
- Validate continuously against the checklist above

**Quality standards:**
- Complete translation (every section, every narrative, every citation)
- Exact name matching (alphaName, stateName, activitySpaceName, etc.)
- Schema compliance (all required fields, correct types, minimum items)
- No hallucination (if it's not in the baseline or report, it doesn't exist)
